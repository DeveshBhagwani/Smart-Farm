...
function updateMissionHeader(preset, obstacles) {
    const startNode = document.getElementById('start-node');
    const endNode = document.getElementById('end-node');

    if (startNode) startNode.value = gridLabel(preset.start);
    if (endNode) endNode.value = gridLabel(preset.goal);

    logMission(`${preset.label} loaded with ${obstacles.length} obstacles.`);
}

function generateDynamicObstacles(preset) {
    const blocked = new Set([cellKey(preset.start), cellKey(preset.goal)]);
    const obstacles = [];
    const count = preset.obstacles.length + 2;

    while (obstacles.length < count) {
        const cell = {
            x: Math.floor(Math.random() * PATH_PLANNER_GRID_SIZE),
            y: Math.floor(Math.random() * PATH_PLANNER_GRID_SIZE)
        };
    ...
    plannerState.obstacleSeed += 1;
    return obstacles.sort((a, b) => (a.y - b.y) || (a.x - b.x));
}
...
function logMission(message) {
    ...
    entry.className = 'mission-log-item';
...
function gridLabel(cell) {
    const column = String.fromCharCode(65 + cell.x);
    return `${column}${cell.y + 1}`;
}
...
function drawStats(plan) {
    ...
    logMission(
        plan.success
            ? `Route found in ${plan.explored} explored cells with ${plan.turns} turns.`
            : 'No safe route found. Try shuffling the obstacles or switching mission mode.'
    );
}
