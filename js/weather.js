...
function displayWeatherData(data) {
    ...
    const safeCity = window.SmartFarm.escapeHtml(data.city);
    const safeCondition = window.SmartFarm.escapeHtml(data.condition);

    weatherResult.innerHTML = `
        <div class="weather-card info-card">
            <h3><i class="fas fa-map-marker-alt"></i> ${safeCity}</h3>
            <div class="weather-main">
                <img src="${iconUrl}" alt="${safeCondition}" style="width:100px; height:100px; margin: 0 auto;">
                <div class="temperature">${data.temperature}°C</div>
                <div class="condition" style="text-transform: capitalize;">${safeCondition}</div>
            </div>
